...
use crate::internals::data_structures::database_metadata::table_data::{GenericDataSQLServer::SmallInt, GenericDatasetDBMS::SQLSERVER};
        ...
        let output = content
            ...
            .map(|data| match data {
                SQLSERVER(dataset) => match dataset {
                    GenericDataSQLServer::Text(text) => text.as_ref().unwrap_or(&String::new()).to_string(),
                    ...
                    GenericDataSQLServer::DateTimeLocal(dtlocal) => {
                        let default_value: NaiveDateTime =
